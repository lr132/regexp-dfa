...
async function init() {
  ...
  try {
    ...
    const wasi = {
      ...
      fd_write: (fd, iovs, iovs_len, nwritten) => {
        ...
        for (let i = 0; i < iovs_len; i++) {
          ...
          const bytes = new Uint8Array(mem.buffer, base, len);
    ...
    console.log("jsffi imports:", Object.keys(jsffiImports));
...
async function doCompile() {
  ...
  try {
    ...
    // Meta
    document.getElementById("meta-states").innerHTML = `<strong>${data.states}</strong> states`;
    document.getElementById("meta-alpha").innerHTML = `Alphabet: <strong>{${data.alphabet.split("").join(", ")}}</strong>`;
    document.getElementById("meta-accept").innerHTML = `Accepting: <strong>{${data.accepting.join(", ")}}</strong>`;

    // Examples
    const exRow = document.getElementById("examples-row");
    exRow.textContent = data.examples.length === 0
      ? "No matches up to length 8."
      : "Examples: " + data.examples.map(s => `<code>${s === "" ? "ε" : s}</code>`).join(", ");
    ...
    showStatus("Compile failed: " + e.message, "error");
  }
}
...
function renderGraph(dot) {
  try {
    ...
    svg.querySelectorAll("text").forEach(t => t.setAttribute("fill", "#e2e8f0"));
...
exportBtn.addEventListener("click", () => {
  if (!lastDot) return;
  const content = `regexp: ${lastRegexp}\n${tablePre.textContent}`;
